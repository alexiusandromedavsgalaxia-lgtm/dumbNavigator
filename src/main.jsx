...
function App(){
 ...
 const navigate=url=>{const u=normalize(url);setTabs(ts=>ts.map((t,i)=>i===active?{...t,url:u,history:[...t.history.slice(0,t.index+1),...(t.url===u?[]:[u])],index:t.url===u?t.index:t.index+1}:t));setAddress(u);setMenu(false);const h=read('dumbHistory',[]).filter(x=>x!==u);h.push(u);write('dumbHistory',h.slice(-100))}
 ...
 return <div className="app">
   ...
   <div className="toolbar"><div className="actions"><button className="icon" disabled={!current.index} onClick={back}>←</button><button className="icon" disabled={current.index>=current.history.length-1} onClick={forward}>→</button><button className="icon" onClick={()=>navigate(HOME)}>⌂</button><button className="icon" onClick={()=>navigate(current.url)}>↻</button></div><form className="address" onSubmit={e=>{e.preventDefault();navigate(address)}}><span>⌕</span><input value={address} onChange={e=>setAddress(e.target.value)} spellCheck="false"/></form><div className="actions"><button className="icon" onClick={bookmark}>{bookmarked?'★':'☆'}</button><button className="icon" onClick={()=>setMenu(v=>!v)}>☰</button></div></div>
   <main className="main"><Browser url={current.url} navigate={navigate}/>{menu&&<div className="panel"><h2>dumbNavigator</h2><div className="menuGrid"><button onClick={()=>alert(read('dumbBookmarks',[]).map(x=>x.url).join('\n')||'no tienes marcadores')}>★ marcadores</button><button onClick={()=>alert(read('dumbHistory',[]).slice().reverse().join('\n')||'historial vacío')}>◷ historial</button><button onClick={()=>{navigator.clipboard?.writeText(current.url);setMenu(false)}}>▣ copiar dirección</button><button onClick={()=>{localStorage.removeItem('dumbHistory');setMenu(false)}}>⌫ borrar historial</button><button onClick={()=>{setMenu(false);navigate('dumb://une.developeit.dev/new')}}>✦ crear una web</button><button onClick={()=>navigate('dumb://une.developeit.dev/about')}>ⓘ acerca de</button><button onClick={()=>navigate('dumb://une.developeit.dev/help')}>❔ ayuda</button><button onClick={()=>navigate('dumb://une.developeit.dev/docs')}>▤ documentación</button></div></div>}</main>
 </div>
}
