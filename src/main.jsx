...
function Creator({projects,setProjects,navigate,initialProject}){
 ...
 const publish=async()=>{setPublishing(true);try{await save();const token=publicToken(project.domain);const res=await fetch('/api/sites',{method:'POST',headers:{'Content-Type':'application/json',...(token?{'x-publish-token':token}:{})},body:JSON.stringify({name:project.name,domain:project.domain,type:project.type,entry:project.entry,files:encodePublicFiles(project.files)})});const data=await res.json();if(!res.ok)throw Error(data.error||'no se pudo publicar');if(data.token)savePublicToken(project.domain,data.token);setPublishedUrl(data.publicUrl);alert('Publicado ✓\\n'+data.publicUrl)}catch(e){alert(e.message||'no se pudo publicar')}finally{setPublishing(false)}}
