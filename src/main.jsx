...
const visibleUrl=url=>{
  ...
  try{const u=new URL(url);return u.protocol==='dumb:'?u.hostname.toLowerCase()+(u.pathname&&u.pathname!=='/'?u.pathname:'')+(u.search||'')+(u.hash||''):''}catch{return ''}
}
const normalize=input=>{
  let s=input.trim()
  if(!s)return DEFAULT_HOME
  if(/^https?:\/\//i.test(s))return DEFAULT_HOME
...
const titleFor=url=>url===DEFAULT_HOME?'Nueva pestaña':url===DEVELOPE?'DevelopE.it':visibleUrl(url)||'Nueva página'
const rewrite=(html,baseUrl='dumb://home')=>html
  .replace(/href\s*=\s*["'](dumb:\/\/[^"']+)["']/gi,(_,u)=>'href="#" data-nav="'+u.replaceAll('"','&quot;')+'"')
  .replace(/href\s*=\s*["'](\/[^"']*)["']/gi,(_,p)=>{try{const u=new URL(baseUrl);return 'href="#" data-nav="'+('dumb://'+u.hostname+(p||'/')).replaceAll('"','&quot;')+'"'}catch{return 'href="#"'}})
  .replace(/href\s*=\s*["'](https?:\/\/[^"']+)["']/gi,()=> 'href="#" data-blocked="external"')

const LANG={
es:{settings:'Configuración',newTab:'Nueva pestaña',home:'Inicio',create:'Crear una web',bookmarks:'Marcadores',history:'Historial',clearHistory:'Borrar historial',language:'Idioma',appearance:'Apariencia',style:'Estilo',background:'Fondo',accent:'Color de acento',density:'Densidad',shape:'Forma de interfaz',save:'Guardar cambios',start:'Empezar a navegar',yourBrowser:'TU NAVEGADOR',what:'¿qué quieres',today:'hacer hoy?',address:'Escribe una dirección',yourSites:'Tus webs',site:'tu sitio',noBookmarks:'Aún no tienes marcadores.',historyEmpty:'Aún no hay historial.',customize:'Haz que el navegador se sienta como tuyo.',languageDesc:'Idioma de la interfaz del navegador.',createDesc:'Construye un sitio HTML local y guárdalo dentro de este navegador.',historyDesc:'Vuelve rápidamente a lo que estabas viendo.',notFound:'Página no encontrada',notFoundDesc:'Esta dirección todavía no existe en dumbNavigator.',backHome:'Volver al inicio',creator:'DUMBNAVIGATOR CREATOR',createTitle:'Crea una web local.',createIntro:'Escribe HTML y guárdalo dentro de este navegador. No se conecta a páginas externas.',preview:'Vista previa',editable:'editable',html:'HTML',domain:'nombre-de-tu-web',publish:'Guardar en este navegador',localBackend:'Funciones locales',localBackendDesc:'Las funciones externas no forman parte de este navegador local.',solid:'Sólido',glass:'Glass',minimal:'Minimal',gradient:'Degradado',plain:'Liso',aurora:'Aurora',comfortable:'Cómoda',compact:'Compacta',spacious:'Amplia',soft:'Suave',sharp:'Precisa',pill:'Redondeada'},
en:{settings:'Settings',newTab:'New tab',home:'Home',create:'Create a website',bookmarks:'Bookmarks',history:'History',clearHistory:'Clear history',language:'Language',appearance:'Appearance',style:'Style',background:'Background',accent:'Accent color',density:'Density',shape:'Interface shape',save:'Save changes',start:'Start browsing',yourBrowser:'YOUR BROWSER',what:'what do you want',today:'to do today?',address:'Enter a website address',yourSites:'Your websites',site:'your site',noBookmarks:'You do not have any bookmarks yet.',historyEmpty:'There is no history yet.',customize:'Make the browser feel like yours.',languageDesc:'Browser interface language.',createDesc:'Build a local HTML site and save it inside this browser.',historyDesc:'Quickly return to what you were viewing.',notFound:'Page not found',notFoundDesc:'This address does not exist in dumbNavigator yet.',backHome:'Back to home',creator:'DUMBNAVIGATOR CREATOR',createTitle:'Create a local website.',createIntro:'Write HTML and save it inside this browser. It never connects to external pages.',preview:'Preview',editable:'editable',html:'HTML',domain:'your-site-name',publish:'Save in this browser',localBackend:'Local functions',localBackendDesc:'External functions are not part of this local browser.',solid:'Solid',glass:'Glass',minimal:'Minimal',gradient:'Gradient',plain:'Plain',aurora:'Aurora',comfortable:'Comfortable',compact:'Compact',spacious:'Spacious',soft:'Soft',sharp:'Sharp',pill:'Rounded'},
de:{settings:'Einstellungen',newTab:'Neuer Tab',home:'Startseite',create:'Website erstellen',bookmarks:'Lesezeichen',history:'Verlauf',clearHistory:'Verlauf löschen',language:'Sprache',appearance:'Darstellung',style:'Stil',background:'Hintergrund',accent:'Akzentfarbe',density:'Dichte',shape:'Form',save:'Änderungen speichern',start:'Surfen starten',yourBrowser:'DEIN BROWSER',what:'was möchtest du',today:'heute tun?',address:'Webadresse eingeben',yourSites:'Deine Websites',site:'deine Seite',noBookmarks:'Noch keine Lesezeichen.',historyEmpty:'Noch kein Verlauf.',customize:'Passe den Browser an.',languageDesc:'Sprache der Browseroberfläche.',createDesc:'Erstelle eine lokale HTML-Seite und speichere sie in diesem Browser.',historyDesc:'Kehre schnell zur letzten Seite zurück.',notFound:'Seite nicht gefunden',notFoundDesc:'Diese Adresse existiert noch nicht.',backHome:'Zur Startseite',creator:'DUMBNAVIGATOR CREATOR',createTitle:'Lokale Website erstellen.',createIntro:'Schreibe HTML und speichere es in diesem Browser. Keine externen Seiten.',preview:'Vorschau',editable:'bearbeitbar',html:'HTML',domain:'deine-seite',publish:'In diesem Browser speichern',localBackend:'Lokale Funktionen',localBackendDesc:'Externe Funktionen gehören nicht zu diesem lokalen Browser.',solid:'Vollton',glass:'Glass',minimal:'Minimal',gradient:'Verlauf',plain:'Einfarbig',aurora:'Aurora',comfortable:'Komfortabel',compact:'Kompakt',spacious:'Großzügig',soft:'Weich',sharp:'Präzise',pill:'Rund'},
fr:{settings:'Réglages',newTab:'Nouvel onglet',home:'Accueil',create:'Créer un site',bookmarks:'Favoris',history:'Historique',clearHistory:'Effacer l’historique',language:'Langue',appearance:'Apparence',style:'Style',background:'Arrière-plan',accent:'Couleur d’accent',density:'Densité',shape:'Forme',save:'Enregistrer',start:'Commencer',yourBrowser:'TON NAVIGATEUR',what:'que veux-tu',today:'faire aujourd’hui ?',address:'Saisis une adresse',yourSites:'Tes sites',site:'ton site',noBookmarks:'Aucun favori pour le moment.',historyEmpty:'Aucun historique.',customize:'Personnalise ton navigateur.',languageDesc:'Langue de l’interface.',createDesc:'Crée un site HTML local et enregistre-le dans ce navigateur.',historyDesc:'Reviens rapidement à ce que tu consultais.',notFound:'Page introuvable',notFoundDesc:'Cette adresse n’existe pas encore.',backHome:'Retour à l’accueil',creator:'DUMBNAVIGATOR CREATOR',createTitle:'Créer un site local.',createIntro:'Écris du HTML et enregistre-le dans ce navigateur. Aucune page externe.',preview:'Aperçu',editable:'modifiable',html:'HTML',domain:'nom-de-ton-site',publish:'Enregistrer dans ce navigateur',localBackend:'Fonctions locales',localBackendDesc:'Les fonctions externes ne font pas partie de ce navigateur local.',solid:'Uni',glass:'Glass',minimal:'Minimal',gradient:'Dégradé',plain:'Uni',aurora:'Aurore',comfortable:'Confortable',compact:'Compact',spacious:'Spacieux',soft:'Doux',sharp:'Précis',pill:'Arrondi'}
};
...
const Home=({navigate,openSettings,config})=>{
  ...
  return <div className="startPage">
    ...
    <footer className="startFooter"><span>Privado · local-first</span><span>Solo webs creadas aquí</span></footer>
  </div>
}
...
function Creator({navigate,config}){
  ...
  return <div className="creatorPro">
    ...
    <div className="publishBar"><span className="protocolBadge">local</span><input value={domain} onChange={e=>setDomain(e.target.value)} placeholder="algo.tudominio.tld o tudominio.tld/algo"/><button className="primary" onClick={publish}>Guardar web →</button></div>
  </div>
}
function BrowserPage({url,navigate,reloadToken,config}){
  ...
  useEffect(()=>{
    ...
    if(site)setHtml(rewrite(site.html,url))
...
function App(){
  ...
  const [active,setActive]=useState(0),[menu,setMenu]=useState(false),[address,setAddress]=useState(''),[reloadToken,setReloadToken]=useState(0),[settingsOpen,setSettingsOpen]=useState(false)
  ...
  return <div className={'app '+(config.style||'solid')+' density-'+(config.density||'comfortable')+' radius-'+(config.radius||'soft')+' bg-'+(config.background||'aurora')} style={{'--accent':config?.color||'#63d6a5'}}>
    <div className="tabs"><div className="windowDots"><i/><i/><i/></div>{tabs.map((t,i)=><div className={'tab '+(i===active?'active':'')} key={i} onClick={()=>setActive(i)}><span className="tabSite">{visibleUrl(t.url)||'Nueva pestaña'}</span><button onClick={e=>{e.stopPropagation();close(i)}}>×</button></div>)}<button className="newtab" onClick={newTab}>＋</button><div className="tabsSpacer"/></div>
    <div className="toolbar"><div className="actions"><button className="icon" disabled={!current.index} onClick={back}>←</button><button className="icon" disabled={current.index>=current.history.length-1} onClick={forward}>→</button><button className="icon" onClick={()=>navigate(DEFAULT_HOME)}>⌂</button><button className="icon" onClick={()=>setReloadToken(x=>x+1)}>↻</button></div><form className="address" onSubmit={e=>{e.preventDefault();navigate(address)}}><span className="addressLock">◉</span><input value={address} onChange={e=>setAddress(e.target.value)} placeholder={tr(config.language,'address')} spellCheck="false"/></form><div className="actions"><button className="icon" onClick={()=>{const a=read('dumbBookmarks',[]);const i=a.findIndex(x=>x.url===current.url);i>=0?a.splice(i,1):a.push({url:current.url,title:current.title});write('dumbBookmarks',a);setReloadToken(x=>x+1)}}>{bookmarked?'★':'☆'}</button><button className="icon" onClick={()=>setMenu(!menu)}>☰</button></div></div>
