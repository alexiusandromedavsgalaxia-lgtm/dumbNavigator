...
import {startProject,stopProject,onRuntimeReady,getRuntimeUrl,buildProjectForPublish} from './runtime'

const HOME='dumb://home'
const CREATE='dumb://create'
const RESERVED=new Set(['home','create','bookmarks','history','projects','settings'])
...
const internalLabel=url=>{const d=domain(url);return ({home:'dumbNavigator · inicio',projects:'dumbNavigator · proyectos',bookmarks:'dumbNavigator · marcadores',settings:'dumbNavigator · ajustes',create:'dumbNavigator · crear'})[d]||visible(url)}
...
const publicTokens=()=>read('dumbPublishTokens',{})
const savePublicToken=(domain,token)=>{const tokens=publicTokens();tokens[domain]=token;write('dumbPublishTokens',tokens)}
const publicToken=domain=>publicTokens()[domain]||''
const encodePublicFiles=files=>Object.fromEntries(Object.entries(files).map(([path,value])=>[path,typeof value==='string'?value:{data:Array.from(fileData(value)),mime:value?.mime||mime(path)}]))
...
function Home({projects,navigate}){
 ...
 return <div className="page homePage"><div className="homeHero"><span className="eyebrow">DUMBNAVIGATOR 2.0</span><h1>el navegador<br/><em>que se construye</em> contigo</h1><p>pestañas, espacios, proyectos locales y un estudio web integrado. todo en una sola app.</p><form className="homeSearch" onSubmit={e=>{e.preventDefault();const q=query.trim();if(!q)return navigate(HOME);const local=projects.find(p=>(p.name+' '+p.domain).toLowerCase().includes(q.toLowerCase()));navigate(local?'dumb://'+local.domain:'https://www.google.com/search?q='+encodeURIComponent(q))}}><Icon name="⌕"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="buscar en tus webs o en internet"/><button type="submit">→</button></form></div><div className="quickGrid"><button className="quick featured" onClick={()=>navigate(CREATE)}><span>✦</span><b>nuevo proyecto</b><small>crea una web desde cero</small><strong>＋</strong></button><button className="quick" onClick={()=>navigate(CREATE)}><span>⌘</span><b>develope</b><small>editor + terminal + preview</small><strong>↗</strong></button><button className="quick" onClick={()=>navigate('dumb://bookmarks')}><span>☆</span><b>marcadores</b><small>tus sitios favoritos</small><strong>↗</strong></button></div><section className="siteSection"><header><div><span className="eyebrow">LOCAL INTERNET</span><h2>tus webs</h2></div><span className="count">{projects.length}</span></header>{filtered.length?<div className="siteGrid">{filtered.map((p,i)=><button className="siteCard" key={p.id} onClick={()=>navigate('dumb://'+p.domain)}><span className="siteNo">{String(i+1).padStart(2,'0')}</span><span className="siteAvatar">{p.name.slice(0,1).toUpperCase()}</span><span className="siteInfo"><b>{p.name}</b><small>{p.domain} · {p.type}</small></span><span>↗</span></button>)}</div>:<div className="empty"><span>◎</span><div><b>{query?'no hay coincidencias':'todavía no hay webs'}</b><p>{query?'prueba otro nombre o dominio':'crea una o importa un repo de github para empezar'}</p></div></div>}</section></div>
}
...
function Creator({projects,setProjects,navigate,initialProject}){
 ...
 useEffect(()=>{let cancelled=false;fetch('/api/sites/'+encodeURIComponent(project.domain)).then(r=>r.ok?r.json():null).then(data=>{if(!cancelled&&data?.published)setPublishedUrl(data.publicUrl)}).catch(()=>{});return()=>{cancelled=true}},[project.domain])
 ...
 const publish=async()=>{setPublishing(true);let restart=false;try{await save();let files=project.files,entry=project.entry;if(isRuntime){if(running){await stopProject();setRunning(false);setRuntimeUrl('');restart=true}const built=await buildProjectForPublish(project,x=>setLogs(l=>[...l,String(x)].slice(-100)));files=built.files;entry=built.entry}const token=publicToken(project.domain);const res=await fetch('/api/sites',{method:'POST',headers:{'Content-Type':'application/json',...(token?{'x-publish-token':token}:{})},body:JSON.stringify({name:project.name,domain:project.domain,type:'static',entry,files:encodePublicFiles(files)})});const data=await res.json();if(!res.ok)throw Error(data.error||'no se pudo publicar');if(data.token)savePublicToken(project.domain,data.token);setPublishedUrl(data.publicUrl);alert('Publicado ✓\\n'+data.publicUrl)}catch(e){alert(e.message||'no se pudo publicar')}finally{setPublishing(false);if(restart){setLogs([]);setRunning(true);try{await startProject(project,x=>setLogs(l=>[...l,String(x)].slice(-100)))}catch(e){setRunning(false);setLogs(l=>[...l,'ERROR: '+e.message])}}}}
 const unpublish=async()=>{const token=publicToken(project.domain);if(!token)return alert('Este dominio se publicó desde otro navegador y no tienes su token.');if(!confirm('¿Despublicar esta web?'))return;setPublishing(true);try{const res=await fetch('/api/sites/'+encodeURIComponent(project.domain),{method:'DELETE',headers:{'x-publish-token':token}});const data=await res.json();if(!res.ok)throw Error(data.error||'no se pudo despublicar');const tokens=publicTokens();delete tokens[project.domain];write('dumbPublishTokens',tokens);setPublishedUrl('')}catch(e){alert(e.message||'no se pudo despublicar')}finally{setPublishing(false)}}
 const stop=async()=>{await stopProject();setRunning(false);setRuntimeUrl('')}
 const isRuntime=isRuntimeType(project)
 return <div className="studio"><aside className="studioSide"><div className="studioBrand"><div className="studioLogo">d</div><div><b>develope</b><small>local studio</small></div></div><label className="field">DOMINIO<input value={project.domain} onChange={e=>setProject({...project,domain:e.target.value.toLowerCase().replace(/[^a-z0-9.-]/g,'')})}/></label><div className="fileActions"><button onClick={()=>fileInput.current?.click()}>＋ archivo</button><button onClick={()=>folderInput.current?.click()}>＋ carpeta</button><button onClick={importGithub}>⌘ github</button><input hidden ref={fileInput} type="file" multiple onChange={e=>addFiles([...e.target.files])}/><input hidden ref={folderInput} type="file" webkitdirectory="" directory="" multiple onChange={e=>addFiles([...e.target.files])}/></div><div className="tree">{Object.keys(project.files).sort().map(f=><button className={selected===f?'active':''} key={f} onClick={()=>setSelected(f)}><span>{ext(f)==='js'||ext(f)==='jsx'?'◈':ext(f)==='css'?'◇':ext(f)==='html'?'□':'·'}</span>{f}</button>)}</div><div className="sideBottom"><button className="ghost" onClick={()=>navigate(HOME)}>← navegador</button><button className="primary" onClick={save}>guardar</button></div></aside><main className="studioMain"><header className="studioHeader"><div><span className="eyebrow">PROJECT / {project.type.toUpperCase()}</span><h2>{project.name}</h2></div><div className="studioBtns"><button className="ghost" onClick={()=>navigate('dumb://'+project.domain)}>abrir</button><button className="primary" disabled={publishing} onClick={publish}>{publishing?'publicando…':'↑ publicar'}</button>{isRuntime&&!running&&<button className="primary" onClick={run}>▶ ejecutar</button>}{isRuntime&&running&&<button className="stop" onClick={stop}>■ detener</button>}{publishedUrl&&<button className="ghost" disabled={publishing} onClick={unpublish}>despublicar</button>}</div></header><div className="editorGrid"><section className="editorPane"><div className="paneHead"><span>{selected}</span><small>{typeof project.files[selected]==='string'?'UTF-8':'BINARY'}</small></div><textarea value={code} onChange={e=>updateCode(e.target.value)} spellCheck="false" disabled={typeof project.files[selected]!=='string'} placeholder="selecciona un archivo de texto"/></section><section className="previewPane"><div className="paneHead"><span>PREVIEW</span><small>{publishedUrl?'PUBLICADO':running?'RUNNING':'IDLE'}</small></div>{running&&runtimeUrl&&isRuntime?<iframe title="runtime" src={runtimeUrl} className="runtimeFrame"/>:<iframe title="static" srcDoc={selected.endsWith('.html')?rewriteStatic(code,project,'/'):('<pre style="padding:20px">'+String(code).replaceAll('<','&lt;')+'</pre>')} className="runtimeFrame"/>}</section></div>{logs.length>0&&<pre className="terminal">{logs.join('')}</pre>}</main></div>
}

function Browser({url,projects,navigate}){
 const p=projects.find(x=>x.domain===domain(url))
 const [src,setSrc]=useState('')
 const [publicState,setPublicState]=useState('idle')
 useEffect(()=>{let cancelled=false;if(p){setPublicState('idle');if(isRuntimeType(p)){setSrc(getRuntimeUrl());return}const path=pathOf(url).replace(/^\//,'')||p.entry;const file=p.files[path]??p.files[p.entry];if(file!==undefined){const value=ext(path)==='html'&&typeof file==='string'?rewriteStatic(file,p,'/'+path.split('/').slice(0,-1).join('/')):file;setSrc(fileUrl(path,value))}return}setSrc('');setPublicState('loading');fetch('/api/sites/'+encodeURIComponent(domain(url))).then(r=>r.ok?r.json():null).then(data=>{if(cancelled)return;if(data?.published){setPublicState('published');setSrc('/sites/'+encodeURIComponent(domain(url))+'/')}else setPublicState('missing')}).catch(()=>{if(!cancelled)setPublicState('missing')});return()=>{cancelled=true}},[url,p])
 ...
 if(!p&&publicState==='published')return <iframe title={visible(url)} src={src} className="siteFrame" sandbox="allow-scripts allow-forms allow-modals"/>
 if(!p&&publicState==='loading')return <div className="notFound"><span>PUBLIC WEB</span><h1>cargando…</h1><p>comprobando si esta web está publicada.</p></div>
 if(!p)return <div className="notFound"><span>404 / LOCAL</span><h1>esta web no existe aquí</h1><p>ese dominio no está creado o publicado en dumbNavigator</p><button className="primary" onClick={()=>navigate(CREATE)}>crear / importar web</button></div>
...
function App(){
 const savedBrowser=read('dumbBrowserState',null)
 const initialTabs=savedBrowser?.tabs?.length?savedBrowser.tabs:[{id:crypto.randomUUID(),url:HOME}]
 const initialActive=Math.min(savedBrowser?.activeTab||0,initialTabs.length-1)
 const [setup,setSetup]=useState(()=>read('dumbSetup',null)),[projects,setProjects]=useState([]),[url,setUrl]=useState(savedBrowser?.url||initialTabs[initialActive]?.url||HOME),[address,setAddress]=useState(''),[menu,setMenu]=useState(false),[sidebar,setSidebar]=useState(false),[tabs,setTabs]=useState(initialTabs),[activeTab,setActiveTab]=useState(initialActive),[accent]=useState(()=>read('dumbAccent','#8b7cff')), [navHistory,setNavHistory]=useState(()=>savedBrowser?.navHistory||[savedBrowser?.url||HOME]),[navIndex,setNavIndex]=useState(()=>Math.min(savedBrowser?.navIndex??0,Math.max(0,(savedBrowser?.navHistory?.length||1)-1)))
 useEffect(()=>{document.documentElement.style.setProperty('--accent',accent);getProjects().then(setProjects)},[accent])
 useEffect(()=>setAddress(internalLabel(url)),[url])
 useEffect(()=>{write('dumbBrowserState',{tabs,activeTab,url,navHistory,navIndex})},[tabs,activeTab,url,navHistory,navIndex])
 const current=useMemo(()=>tabs[activeTab],[tabs,activeTab])
 const navigate=useCallback(next=>{const u=normalize(next);setUrl(u);setTabs(ts=>ts.map((t,i)=>i===activeTab?{...t,url:u}:t));setNavHistory(h=>{const base=h.slice(0,navIndex+1);const nextHistory=[...base,u];setNavIndex(nextHistory.length-1);return nextHistory});setMenu(false);const history=read('dumbHistory',[]);write('dumbHistory',[...history.filter(x=>x!==u),u].slice(-100))},[activeTab,navIndex])
 const submit=e=>{e.preventDefault();const q=address.trim();if(!q||q.startsWith('dumbNavigator ·')){navigate(HOME);return}const clean=q.replace(/^https?:\/\//i,'').replace(/\/$/,'').toLowerCase();const local=projects.find(p=>p.domain.toLowerCase()===clean||p.name.toLowerCase()===q.toLowerCase());navigate(local?'dumb://'+local.domain:q)}
 const newTab=()=>{const t={id:crypto.randomUUID(),url:HOME};setTabs(ts=>[...ts,t]);setActiveTab(tabs.length);setUrl(HOME);setNavHistory([HOME]);setNavIndex(0)}
 ...
 const selectTab=i=>{setActiveTab(i);setUrl(tabs[i].url);setNavHistory([tabs[i].url]);setNavIndex(0)}
 ...
 const page=url===HOME?'home':path==='projects'?'projects':path==='bookmarks'?'bookmarks':path==='settings'?'settings':path==='create'?'creator':'browser'
 return <div className="app"><header className="browserChrome"><div className="traffic"><i/><i/><i/></div><button className="brandBtn" onClick={goHome}>d</button><div className="tabs">{tabs.map((t,i)=><button key={t.id} className={'tab '+(i===activeTab?'active':'')} onClick={()=>selectTab(i)}><span>{internalLabel(t.url)}</span>{tabs.length>1&&<b onClick={e=>{e.stopPropagation();closeTab(i)}}>×</b>}</button>)}<button className="newTab" onClick={newTab}>＋</button></div><button className="windowBtn" onClick={()=>setMenu(v=>!v)}>☰</button></header><div className="toolbar"><div className="navButtons"><button disabled={navIndex<=0} onClick={()=>{const i=navIndex-1;const u=navHistory[i];setNavIndex(i);setUrl(u);setTabs(ts=>ts.map((t,n)=>n===activeTab?{...t,url:u}:t))}}>‹</button><button disabled={navIndex>=navHistory.length-1} onClick={()=>{const i=navIndex+1;const u=navHistory[i];setNavIndex(i);setUrl(u);setTabs(ts=>ts.map((t,n)=>n===activeTab?{...t,url:u}:t))}}>›</button><button onClick={goHome}>⌂</button></div><form className="address" onSubmit={submit}><span>⌕</span><input value={address} onChange={e=>setAddress(e.target.value)} placeholder="buscar o escribir un dominio"/><button type="button" onClick={toggleBookmark}>☆</button><button type="submit">↵</button></form><div className="toolbarRight"><button onClick={()=>setSidebar(v=>!v)}>☷</button><button onClick={()=>setMenu(v=>!v)}>⋯</button></div></div>{menu&&<Menu navigate={navigate} close={()=>setMenu(false)} projects={projects}/>}<main className="browserBody">{page==='home'?<Home projects={projects} navigate={navigate}/>:page==='projects'?<Projects projects={projects} setProjects={setProjects} navigate={navigate}/>:page==='bookmarks'?<Bookmarks navigate={navigate}/>:page==='settings'?<Settings navigate={navigate}/>:<Browser url={url} projects={projects} navigate={navigate}/>}</main>{sidebar&&<aside className="sidePanel"><header><b>sidebar</b><button onClick={()=>setSidebar(false)}>×</button></header><button onClick={()=>navigate('dumb://bookmarks')}>☆ <span>marcadores</span></button><button onClick={()=>navigate('dumb://projects')}>▦ <span>proyectos</span></button><button onClick={()=>navigate(CREATE)}>⌘ <span>develope</span></button><div className="sideHint">dumbNavigator<br/><small>tu internet local</small></div></aside>}</div>
}
