...
function emit(url){activeUrl=url;for(const fn of readyListeners)fn(url)}
...
function tree(files){
  ...
  for(const [path,value] of Object.entries(files)){
    ...
    parts.forEach((part,i)=>{
      if(i===parts.length-1)node[part]={file:{contents:value}}
      else{node[part]??={directory:{}};node=node[part].directory}
    })
  }
  return root
}
export async function startProject(project,onLog=()=>{}){
  ...
  if(activeProcess){try{activeProcess.kill()}catch{};activeProcess=null}
  ...
  const pkg=project.files['package.json']?JSON.parse(project.files['package.json']):null
  if(!pkg){throw new Error('Este proyecto no tiene package.json.')}
  ...
  if(installCode!==0)throw new Error('npm install terminó con código '+installCode)
  const scripts=pkg.scripts||{}
  const command=scripts.dev?'dev':scripts.start?'start':scripts.serve?'serve':null
  if(!command)throw new Error('No encuentro un script dev, start o serve en package.json.')
  ...
  wc.on('server-ready',(port,url)=>{if(activeProjectId===project.id)emit(url)})
  return {wc,process}
}
